...
- `作文词性统计宽表.xlsx`：项目最终主分析表，基于 `clean_text` 全量生成，当前为620行、392列。`词性统计` sheet 每行对应一篇作文，`字段说明` sheet 逐列记录定义、公式和分母，`语言特征说明` sheet 将392个字段归并为178项语言特征，并以中文列出识别与统计口径、例项和对应字段；后续统计分析应优先使用此表。
...
- `母语作文词性统计宽表.xlsx`：180篇母语参照语料的392列派生统计，字段顺序和口径与学习者主宽表完全一致，并包含同结构的 `字段说明` 和 `语言特征说明` sheet；作文分数留空，国籍统一标记为“中国（公开网络样本）”。
...
- `scripts/add_language_feature_catalog.mjs`：读取两份宽表的 `字段说明`，将相关次数、每千汉字频率、占比和多样性指标归并为178项语言特征，并生成中文 `语言特征说明` sheet。
...
为学习者和母语两份宽表补充或更新 `语言特征说明` sheet：

```bash
node scripts/add_language_feature_catalog.mjs
```

说明表固定使用 `大类、ID、特征中文名、识别与统计口径、例（Example）、宽表对应字段` 六列；特征名称和口径说明只用中文。可用 `--dry-run` 只校验归并结果，或重复传入 `--input` 指定目标工作簿。
